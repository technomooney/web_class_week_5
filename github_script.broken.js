...
analyzeButton.addEventListener('click', function() {
    ...
    let repoPerPage = 50
    let maxPages = null
    let userStatURL = `https://api.github.com/users/${username}`
    let url = `https://api.github.com/users/${username}/repos?per_page=${repoPerPage}&page=${page}`
    let dataJson = {}
    // todo get all repos using pages and process them
    fetch(userStatURL).then(response => response.json()).then(function (gitStatsJson) {
        let numOfUserRepos = gitStatsJson["public_repos"]
        maxPages = Math.ceil(numOfUserRepos / repoPerPage)
        console.log(numOfUserRepos)
        console.log(maxPages)
    })

    for (let i = 0; i < maxPages; i++) {
        console.log(`Started ${i+1} times`)
        let nextPage = getJsonPages(url)
        console.log(nextPage)
        page = i+1
        url = `https://api.github.com/users/${username}/repos?per_page=${repoPerPage}&page=${page}`
        console.log(`ran ${i+1} times`)
    }

    console.log(dataJson)
    displayRepositoriesOnPage(dataJson)
})
